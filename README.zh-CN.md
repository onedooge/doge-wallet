...
| 🎰 **老虎机** | 单机小游戏：游戏币转轮，**不使用真实 DOGE**，仅娱乐 / 测试 |
| 🆔 **Doger ID** | 12 位 Doge 号（从地址推导，类 QQ 号），显示为 `684 419 585 614`。**本地 Demo** |
| 🌐 **.doge 域名** | Doge Name Service：取 `name.doge` + 名册解析，转账免记长地址。**本地 Demo** |
| 💬 **Doge Chat** | 通讯录 + 聊天（内置 Doge 小助手自动回复）。**本地 Demo**，跨用户联网为下一步 |
